...
public sealed class TokenService : ITokenService
{
   private const string Key = "hefi_tokens_v1";
    ...
    public async Task<Token?> LoadAsync() //TODO:del later
    {
        var raw = await SecureStorage.GetAsync(Key);
        System.Diagnostics.Debug.WriteLine($"[TokenService] Load from {Key}: {(string.IsNullOrEmpty(raw) ? "<empty>" : $"{raw.Length} chars")}");
        return string.IsNullOrEmpty(raw) ? null : JsonSerializer.Deserialize<Token>(raw);
    }
